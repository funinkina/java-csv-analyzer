...
public class GeminiService {
        ...
        public String getCleanedCsv(String prompt, List<String[]> csvData) {
                String url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key="
                                + apiKey;

                String csvString = csvData.stream()
                                .map(row -> Arrays.stream(row).collect(Collectors.joining(",")))
                                .collect(Collectors.joining("\n"));

                String fullPrompt = "You are a data cleaning assistant. Given the following CSV data, perform the requested cleaning operation: "
                                + prompt +
                                "\n\nOutput only the cleaned CSV data as plain text, with no explanations or markdown. Ensure the output is valid CSV format.\n\nCSV Data:\n"
                                + csvString;
                ...
                JSONObject jsonResponse = new JSONObject(response);
                String cleanedCsv = jsonResponse.getJSONArray("candidates")
                                .getJSONObject(0).getJSONObject("content")
                                .getJSONArray("parts").getJSONObject(0)
                                .getString("text");

                cleanedCsv = cleanedCsv.replace("```csv", "").replace("```", "").trim();
                return cleanedCsv;
        }
}